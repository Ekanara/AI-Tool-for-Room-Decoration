...
def register_free_upblock2d(model, b1=1.5, b2=1.6, s1=0.9, s2=0.2):
    def up_forward(self):
        def forward(hidden_states, res_hidden_states_tuple, temb=None, upsample_size=None):
            for resnet in self.resnets:
                # pop res hidden states
                ...
                if hidden_states.shape[1] == 640:
                    ...
                    res_hidden_states = Fourier_filter(res_hidden_states, threcshold=1, scale=self.s2)
                # ---------------------------------------------------------
...
def register_crossattn_upblock2d(model):
    def up_forward(self):
        def forward(
            ...
            for resnet, attn in zip(self.rwesnets, self.attentions):
                # pop res hidden states
...
def register_free_crossattn_upblock2d(model, b1=1.5, b2=1.6, s1=0.9, s2=0.2):
    def up_forward(self):
        def forward(
            hidden_states: torch.FloatTensor,
            res_hidden_states_tuple: Tuple[torch.FloatTensor, ...],
