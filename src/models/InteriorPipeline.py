...
class InteriorPipeline:
    def __init__(self, sd_id='stablediffusionapi/interiordesignsuperm', ip_id='models/ip_sd15_64.bin', NUM_DDIM_STEPS=40,
        ...
        self.ip_id = ip_id
        self.image_encoder = CLIPVisionModelWithProjection.from_pretrained('h94/IP-Adapter',
                                                                           subfolder='models/image_encoder').to('cuda',
                                                                                                                dtype=precision)
    ...
    def get_image_embeds(self, pil_image):
        ...
        self.image_encoder(torch.zeros_like(clip_image), output_hidden_states=True).hidden_states[-2].detach()
    ...
    def load_adapter(self, model_path, scale=1.0):
        ...
        for name in self.pipe.unet.attn_processors.keys():
            cross_attention_dim = None if name.endswith(
                "attn1.processor") else self.pipe.unet.config.cross_attention_dim
